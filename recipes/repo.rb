...
when 'suse'
  ...
  suse = shell_out("head -n1 /etc/SuSE-release| awk '{print $1}'").stdout.chomp.downcase
  suse = 'sles' if suse == 'suse'
  suse_version = suse << shell_out("grep VERSION /etc/SuSE-release | awk -F'= ' '{print $2}'").stdout.chomp
