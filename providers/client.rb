...
action :add do
  ...
  if @current_resource.exists
    if @current_resource.keys_match && @current_resource.caps_match
      Chef::Log.info "Client #{@new_resource} already exists and matches specifications - nothing to do."
    else
      converge_by("Recreating client #{@new_resource} as existing doesn't match specifications") do
